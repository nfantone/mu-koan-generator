...
function runNpmInit() {
  return spawn('npm', ['init'], {
    ...
    const original = jsonfile.readFileSync(PACKAGE_JSON);
    const template = jsonfile.readFileSync(path.join(TEMPLATES_DIR, '_package.json'));
    const merged = Object.assign({}, original, template);
    fs.unlinkSync(PACKAGE_JSON);
    jsonfile.writeFileSync(PACKAGE_JSON, merged);
    return res;
  });
}
